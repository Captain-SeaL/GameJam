...
public class AnimationFollowing : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        slaveTransforms = slave.GetComponentsInChildren<Transform>().Where(t => t.name != "CUBE").ToArray(); // Get all transforms in ragdoll.\
        masterTransforms = master.GetComponentsInChildren<Transform>().Where(t => t.name != "CUBE").ToArray(); // Get all transforms in master. 
